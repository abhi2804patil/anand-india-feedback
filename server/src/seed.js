...
function makeSubmission(i) {
  ...
  return {
    ...
    recommendReason: recommends === 'Yes'
      ? 'Good amenities and central location.'
    ...
    likeMost: pick([
      ...
      'Professional environment and clean premises.',
      'Reliable power backup and internet.',
      'Friendly staff and well-maintained common areas.',
      'Safe, secure and family-friendly.',
    ]),
    dislikeMost: pick([
      'Parking is always a struggle.',
      'Afternoon AC cooling is weak.',
      'Pantry options are limited.',
      'Washrooms need more frequent cleaning.',
      'Lift waiting time in mornings.',
    ]),
    ownerChange: pick([
      'Expand meeting room availability.',
      'Introduce a visitor management system.',
      'Better signage inside the building.',
      'Dedicated parking per tenant.',
...
(async () => {
  await connectDb();
  const rows = Array.from({ length: 20 }, (_, i) => makeSubmission(i));
  // Seed by inserting one-by-one so each gets a submissionId + timestamps.
  const existing = await repo.list();
  if (existing.length > 0) {
    console.log(`[seed] already have ${existing.length} rows — wiping and reseeding.`);
    await repo.replaceAll([]);
  }
  for (const r of rows) await repo.create(r);
  console.log(`[seed] inserted ${rows.length} feedback submissions`);
  process.exit(0);
})();
